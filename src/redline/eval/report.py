...
def render_eval_markdown(rows: list[dict], registration: dict | None = None) -> str:
    ...
    out += [
        "## Reproducibility", "",
        "This file is generated by `python -m redline.eval.report` from the "
        "persisted `eval_runs` table. Regenerating the numbers **requires network "
        "access to EDGAR and an LLM API key** (extraction + LLM-judge grading), and "
    ...
    if guidance:
        out += [
            "**8-K guidance panel** — selected by a mechanical rule (Rule R) over "
            "persisted DB state (a pure, deterministic read, no network at selection "
            "time), then extracted and scored against the operational database "
            "(`data/redline.db`, the default). This committed file is that database's "
            "report:", "",
            "```",
            ...
            "```", "",
            "The two blocks target different databases (the isolated `data/eval_run.db` "
            "vs the operational `data/redline.db`) — run whichever section you want to "
            "reproduce.", "",
        ]
    return "\n".join(out) + "\n"
