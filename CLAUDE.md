...
- **DCF valuation layer (Subsystem 7):** BUILT (2026-07-27). Event-driven revaluation from XBRL financials — NOT sentiment, NOT alpha (see §4). Phase 0 gate (`spike/valuation_feasibility.py`, NOTES §6) established two things that shaped it: (a) the XBRL spine is clean for all 8 names via `Company.get_facts()` / `get_financials()`; (b) forward guidance is NOT in the 10-K/10-Q MD&A Redline diffs — it lives in 8-K earnings exhibits — so the narrative-extraction path is **deferred** to a separately-feasibility-gated 8-K follow-on. Shipped: `src/redline/valuation/` (`dcf.py` engine → bear/base/bull ranges + sensitivity; `xbrl.py` companyfacts ingest; `fcf.py` + `models.py` base builder / assumptions / validation; `revalue.py` immutable before/after hook; `validation_eval.py` FCF-base eval) + `config/valuation/{assumptions,fcf_mapping_v1}.yaml` + dashboard valuation section. DCF covers the **6 non-financials** (banks SCHW/KEY excluded — unlevered-FCF DCF is wrong for financials). Assumptions were **verified by Ian** (transcribed 2026-07-28); all six are `is_placeholder: false` (WACC / terminal growth / reference price sourced offline). An `ILLUSTRATIVE — NOT VALIDATED` banner fires on any card whose assumptions are ever reset to placeholder. Validation confirms edgartools FCF and an independent DB reconstruction agree to 0.00%.
- **8-K guidance hook (the differentiated path):** BUILT (2026-07-27, NOTES §7). Feasibility gate PASSED (0.80 of item-2.02 8-Ks carry extractable guidance — it IS in the EX-99.1 earnings exhibits, unlike MD&A). `valuation/guidance.py` (LLM-typed extraction → `extracted_figures`, confidence-gated, with raised/lowered/reaffirmed deltas), `revalue.run_guidance_revaluations` (a filed revenue-guidance figure overrides year-1 growth → before/after link, source=`guidance`), `guidance_eval.py` (precision/recall grader). **Ran live (2026-07-30)** on `claude-sonnet-4-6` via the Anthropic fallover; the panel was expanded to a mechanically-selected, locked **12-accession** set (tag `guidance-eval-registration-v1`) and scored. EVAL.md leads with the pipeline-faithful number — revenue guidance (the only DCF input), trigger-eligible precision/recall **1.000/1.000**; raw all-metrics 0.714 (the honest held-out gap). See NOTES §12 / EVAL.md. ~246 tests total across the project.
