...
**⚠ BLOCKER (RESOLVED 2026-07-28, see §12).** At build time OpenAI returned **429 insufficient_quota** and there was no `ANTHROPIC_API_KEY`, so the guidance path was mock-only. It has since run live on `claude-sonnet-4-6` via the fallover: `guidance_labels.yaml` now carries 28 real curated gold figures, `extracted_figures` is populated, and the extraction eval produced a real measured number (precision 0.875 — see §12 for the final-state summary and the honest precision gap).
...
1. **Guidance precision (RESOLVED 2026-07-30).** The earlier 0.875 counted 3 *correctly-classified segment figures* (e.g. PLTR US-commercial revenue) as false positives against the totals-only frozen gold — the extractor was right, the gold is totals-only. Measured on `scope='total'` (what the gold covers), precision is **0.966** (recall 1.000, F1 0.982; 28 TP / 1 FP / 0 FN, 3 segment figures excluded) — clears the 0.90 bar. `guidance_eval._extracted_rows` now defaults to `scope='total'` and reports the excluded-segment count; the frozen gold is untouched. (The confidence gate also quarantines actual junk.)
   ...
   **Scored 2026-07-30 (Anthropic Sonnet fallover; OpenAI still 429).** Extraction of the 8 never-seen accessions cost ~$0.24 (logged spend $1.76→$2.00, under the $3 cap). Real panel numbers (scope=total, panel-restricted): **precision 0.690, recall 0.967, F1 0.806** (29 TP / 13 FP / 1 FN) — down from the n=3 0.966, which is the honest **generalization gap** a mechanically-selected held-out panel is meant to expose. Per-metric tells the real story: **revenue precision 0.923 / recall 1.000** (the DCF-relevant metric — strong), ebitda 1.000, eps 0.833; the precision drag is concentrated in `other` (0.250) and `comparable_sales` (0.000). Analysis of the 13 FPs: ~10 are the extractor *correctly* emitting real forward guidance that the totals-only / included-metrics scoping deliberately EXCLUDES — VRTX combined R&D+SG&A expense (~$6.3-6.45B) and effective tax rate (19.5-20.5%), MRNA opex, ULTA comparable-sales (2.5-3.5%) and capex — i.e. a gold-*boundary* effect analogous to the segment-figure exclusion, not hallucination (recall stays ~1.0). ~3 are genuine curation gaps to review: ULTA `027061` operating-income-growth omitted from gold, and an ULTA EPS **representation mismatch** (gold recorded `027061` EPS as growth-% 9.4-11.4 but the extractor emitted the equivalent $-level $28.05-28.55, so it scores as 1 FP + the sole FN). Whether to (a) count out-of-scope-metric emissions as FP or exclude them like segment figures, and (b) re-curate the ULTA EPS representation, are open gold-scoping calls — left as-is (no post-hoc tuning of a measured eval). **Percentage-guidance scoping (resolved 2026-07-30, Ian):** ULTA (×2) guides in firm two-sided % ranges → **included** as `unit=pct` gold (consistent with NOTES §7, which already treated ULTA's "net sales growth 6.0%–7.0%" as extractable); MRNA (×2) states only a one-sided soft "plan to deliver up to 10% revenue growth" → **excluded** as an aspiration (zero gold → hallucination test). Comparable-sales growth excluded as an operational metric. Gold now = 36 figures over 9 of the 12 panel accessions (MRNA ×2 + the pre-seen CVNA `-000034` are intentionally zero-gold).
