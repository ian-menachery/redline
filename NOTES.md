...
1. **Guidance precision 0.875 vs the pre-registered 0.90 bar.** Residual false positives are *correctly-classified segment figures* (e.g. PLTR US-commercial revenue) scored against a totals-only frozen gold — the extractor is right, the gold is totals-only. Future: a segment-labeled gold set, or measure precision on `scope=total` figures only. (The confidence gate quarantines actual junk; the `trigger_eligible` subset was 18/18 = 1.00.)
2. **FCF-DCF does not reconcile for growth / turnaround names.** NET (~24×), PLTR (~9×), MRNA (~7× on negative FCF) are structurally un-modelable by unlevered-FCF DCF; they are monitored, not valued. Future: a multiples-based or reverse-DCF approach for growth names.
...
6. **NET mechanism tile copy risk (open for Ian's decision).** The mechanism section carries three explicit disclaimers, but its `st.metric` renders a standalone dollar value labeled "Modeled base (before → after)"; a tile-only skimmer could misread the number as a NET price target. Candidate copy fix: relabel the tile or suppress the absolute figure. Not changed unprompted.
7. **Hosted valuation dashboard visibility.** As of close-out the Streamlit app is **Private** (logged-out visitors are redirected to a sign-in wall), so it is not yet viewable by a recruiter; it must be set Public in the Streamlit console and confirmed in an incognito window before the URL is treated as live.
